...
export function useAppWorkspace(app: AppLike | null, providedProjectId?: number | null) {
  ...
  // The requested workspace takes ownership before the syncing effect runs.
  projectIdRef.current = providedProjectId ?? urlProjectId ?? projectId;
  ...
  const loadAssets = useCallback(async (id: number) => {
    // Also covers an old addImageAsset completion after navigation/unmount.
    ...
    try {
      ...
      if (mountedRef.current && projectIdRef.current === id

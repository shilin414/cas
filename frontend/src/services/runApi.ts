...
const applicationPageInflight = new Map<string, Promise<ApplicationPage>>();

export function fetchApplicationPage(
  ...
  const existing = applicationPageInflight.get(key);
  if (existing) return existing;
  const request = api.get<ApplicationPage>('/v2/applications/page', params);
  applicationPageInflight.set(key, request);
  const clear = () => {
    if (applicationPageInflight.get(key) === request) applicationPageInflight.delete(key);
  };

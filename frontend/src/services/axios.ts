...
const axiosInstance = axios.create({
  // Shared deployment prefix + /api; an explicit VITE_API_BASE_URL must
  // include the complete API base, not just an origin.
  baseURL: apiUrl(),
  timeout: 10000,
...
const redirectToLogin = () => {
  ...
  const returnTo = captureSessionLoginReturnTo();
  useAuthStore.getState().clearAuth();
  navigateToSessionLogin(returnTo);
};

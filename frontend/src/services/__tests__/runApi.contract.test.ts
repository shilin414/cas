...
describe('catalog client URL contract', () => {
  ...
  it('coalesces concurrent identical page requests (StrictMode-safe)', async () => {
    let resolve!: (value: { items: never[]; next_cursor: string; has_more: boolean }) => void;
    axiosMock.get.mockReturnValueOnce(new Promise((done) => { resolve = done; }));
    const first = fetchApplicationPage({ kind: 'chat', limit: 24 });
    const second = fetchApplicationPage({ kind: 'chat', limit: 24 });
    expect(axiosMock.get).toHaveBeenCalledTimes(1);
    resolve({ items: [], next_cursor: '', has_more: false });

...
describe('explicit logout lifecycle', () => {
  ...
  it('routes explicit logout to the manual page, but session expiry to auto OAuth', async () => {
    ...
    const manual = new URL(host.textContent!, 'https://example.test');
    expect(manual.pathname).toBe('/auth/login');
    expect(manual.searchParams.get('logged_out')).toBe('1');
    expect(manual.searchParams.get('return_to')).toBe('/private');
    ...
    const automatic = new URL(host.textContent!, 'https://example.test');
    expect(automatic.pathname).toBe('/login');
    expect(automatic.searchParams.get('logged_out')).toBeNull();
    expect(automatic.searchParams.get('return_to')).toBe('/private');
  });

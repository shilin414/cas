/**
 * AgentEditorModal — 企业管理「智能体管理」的新建 / 编辑入口。
 *
...
const AgentEditorModal = ({
  ...
  const typeOptions = useMemo(() => {
    ...
    return [{ label: runtimeLabel, value: 'runtime' as AgentEditorMode }];
  }, [runtimes]);

  const currentRuntimeUnavailable = Boolean(
    isEdit && managedAgent && !selectedRuntime && !runtimeKey,
  );

  const resourceRules = currentRuntimeUnavailable
    ? []
    : selectedRuntime?.resource_id_required
    ? [{
      required: true,
        ...
        {agentType === 'runtime' ? (
          <>
            <Form.Item
              name="runtime_key"
              label="运行时"
              rules={currentRuntimeUnavailable ? [] : [{ required: true, message: '请选择运行时' }]}
              extra={currentRuntimeUnavailable
                ? '当前绑定的运行时已不可用；保存其他信息不会更改原绑定。若需迁移，请主动选择新的运行时并核对资源 ID。'

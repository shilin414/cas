...
import { createSchedule, fetchSchedule, previewScheduleRuns, updateSchedule } from '@/services/scheduleApi';
...
export function useScheduleEditor({
  ...
  // 列表响应省略 deliveries，不能把「尚未读取」误当成「没有投递」。
  // render-time session identity also rejects late A -> B -> A responses before effect cleanup.
  const hydrationSession = useMemo(() => ({ key: editorSessionKey }), [editorSessionKey]);
  const activeHydrationSession = useRef(hydrationSession);
  activeHydrationSession.current = hydrationSession;
  const needsHydration = editing !== null && editing.deliveries === undefined;
  ...
  const currentHydration = hydration?.session === hydrationSession ? hydration : null;
  const editingDetail = needsHydration ? currentHydration?.detail ?? null : editing;
  const hydrationReady = !needsHydration || editingDetail !== null;
  const hydrationError = needsHydration ? currentHydration?.error ?? null : null;
  const hydrationLoading = open && needsHydration && !hydrationReady && !hydrationError;
  ...
  useEffect(() => {
    if (!open || !needsHydration || editingId === null) return;
    let stale = false;
    const isCurrent = () => !stale && activeHydrationSession.current === hydrationSession;
    setHydration({ session: hydrationSession, detail: null, error: null });
    void (async () => {
      try {
        const detail = await fetchSchedule(editingId);
        if (!isCurrent()) return;
        // Fail closed: an incomplete/partial-success response must never authorize a destructive PATCH.
        if (detail.id !== editingId || !Array.isArray(detail.deliveries)) {
          throw new Error('自动化详情缺少完整投递配置，请重试加载详情');
        }
        setHydration({ session: hydrationSession, detail, error: null });
      } catch (error) {
        if (!isCurrent()) return;
        setHydration({ session: hydrationSession, detail: null, error: error instanceof Error ? error.message : '加载自动化详情失败' });
      }
    })();
    return () => { stale = true; };
  }, [open, needsHydration, editingId, hydrationSession, hydrationAttempt]);
  ...
  const targetApplicationId = (editingDetail ?? editing)?.application_id ?? presetApplicationId ?? null;
  ...
  useEffect(() => {
    if (!open) return;
    if (needsHydration && !editingDetail) {
      form.resetFields();
      setDeliveryOn(false);
      setSelectedTarget(null);
      setPreview([]);
    ...
    if (editingDetail) {
      const v = scheduleToForm(editingDetail);
    ...
    setPreview([]);
  }, [open, editingDetail, needsHydration, presetApplicationId, form]);
  ...
  const userTargets = useFeishuTargets({
    type: 'user',
    enabled: open && hydrationReady && deliveryOn,
    query: targetQuery,
    sessionKey: editorSessionKey,
  });
  ...
  /** 单目标 UI 只替换第一项，保留条件与未展示的历史目标。 */
  const selectDeliveryTarget = useCallback((target: FeishuForwardTarget) => {
    const deliveries: ScheduleFormValues['deliveries'] = form.getFieldValue('deliveries') ?? [];
    form.setFieldValue('deliveries', [{
      ...deliveries[0],
      target_type: target.target_type,
      target_id: target.id,
      target_name: target.name,
      condition: deliveries[0]?.condition ?? { operator: 'always' },
    }, ...deliveries.slice(1)]);
  ...
  const buildPayload = useCallback((values: ScheduleFormValues) => {
    if (deliveryOn && (editingDetail?.deliveries?.length ?? 0) > values.deliveries.length) {
      throw new Error('此自动化包含多个投递目标，不能静默减少目标；请重新打开编辑器后重试');
    }
    const payload = formToPayload({ ...values, deliveries: deliveryOn ? values.deliveries : [] }, { original: editingDetail ?? undefined });
    // PATCH 省略 deliveries 意味着保留，显式 [] 才能关闭通知。
    payload.deliveries ??= [];
    return payload;
  }, [deliveryOn, editingDetail]);

  const reportValidationError = useCallback((error: unknown) => {
  ...
  const refreshPreview = useCallback(async () => {
    if (!open || !hydrationReady) {
      if (open) message.error(hydrationError ? '请重试加载详情后再预览' : '正在加载完整自动化配置，请稍候');
      return;
    }
    // 预览代际（四次复审 P2-5）：请求在途时用户还能继续改执行时间/星期/
    // 时区 —— 旧表单算出的预览晚到后不得覆盖新配置下的结果。
    const seq = ++previewSeqRef.current;
    const previewEpoch = editorEpochRef.current;
    try {
      setPreviewing(true);
      const values = await readValues();
      if (seq !== previewSeqRef.current || previewEpoch !== editorEpochRef.current) return;
      const payload = buildPayload(values);
      const runs = await previewScheduleRuns(payload);
      if (seq !== previewSeqRef.current || previewEpoch !== editorEpochRef.current) return; // 表单或会话已变
      setPreview(runs);
    } catch (error) {
      if (seq === previewSeqRef.current && previewEpoch === editorEpochRef.current) reportValidationError(error);
    } finally {
      // 只有最新一次预览拥有 spinner 状态。
      if (seq === previewSeqRef.current) setPreviewing(false);
    }
  }, [open, hydrationReady, hydrationError, readValues, buildPayload, reportValidationError]);
  ...
  const handleOk = async () => {
    if (!open || !hydrationReady) {
      if (open) message.error(hydrationError ? '请重试加载详情后再保存' : '正在加载完整自动化配置，请稍候');
      return;
    }
    ...
    try {
      setSaving(true);
      const values = await readValues();
      if (saveEpoch !== editorEpochRef.current) return;
      const payload = buildPayload(values);
      ...
      if (editorEpochRef.current === saveEpoch) {
        ...
        if (!(e instanceof Error)) message.error('请检查表单后重试');
      }
    } finally {
  ...
  const watchedScheduleType = Form.useWatch('schedule_type', { form, preserve: true });
  const watchedTriggerTime = Form.useWatch(['trigger', 'time'], { form, preserve: true });
  const watchedDaysOfWeek = Form.useWatch(['trigger', 'days_of_week'], { form, preserve: true });
  const watchedDayOfMonth = Form.useWatch(['trigger', 'day_of_month'], { form, preserve: true });
  const watchedRunAt = Form.useWatch('run_at_local', { form, preserve: true });
  const watchedTimezone = Form.useWatch('timezone', { form, preserve: true });
  const watchedStartMode = Form.useWatch('start_mode', { form, preserve: true });
  const watchedEndMode = Form.useWatch('end_mode', { form, preserve: true });
  const watchedStartsAt = Form.useWatch('starts_at_local', { form, preserve: true });
  const watchedEndsAt = Form.useWatch('ends_at_local', { form, preserve: true });
  useEffect(() => {
    ...
    watchedStartMode, watchedEndMode, watchedStartsAt, watchedEndsAt, editorSessionKey,
  ]);
  ...
  return {
    ...
    setSelectedTarget, selectDeliveryTarget,

...
/** 在用户/群聊统一搜索框输入关键词，等联系人服务端搜索落地。 */
async function searchTargets(query: string) {
  const input = document.querySelector<HTMLInputElement>('input[placeholder="搜索用户或群聊"]');
  expect(input, '未找到统一搜索输入框').toBeTruthy();
...
beforeEach(() => {
  ...
  mocks.loadForwardHistory.mockReset().mockReturnValue([]);
...
describe('FeishuForwardModal — 用户与群聊统一搜索', () => {
  it('shows mixed recent targets first and searches users/groups without tabs', async () => {
    mocks.loadForwardHistory.mockReturnValue([
      target('ou-recent', '最近联系人', 'user'),
      target('oc-recent', '最近群聊', 'chat'),
    ]);
    mocks.fetchFeishuTargets.mockImplementation(
      async (type: 'user' | 'chat') => type === 'chat'
        ? page([target('oc-dev', '研发群', 'chat')])
        : page([target('ou-zhang', '张三', 'user')]),
    );

    await mountModal();

    expect(document.querySelector('.ant-tabs')).toBeNull();
    const input = document.querySelector<HTMLInputElement>('input[placeholder="搜索用户或群聊"]');
    expect(input).toBeTruthy();
    const recent = document.querySelector('.ffm-recent');
    const list = document.querySelector('.ffm-list');
    expect(Boolean(recent && list && (recent.compareDocumentPosition(list) & Node.DOCUMENT_POSITION_FOLLOWING))).toBe(true);
    expect(recent?.textContent).toContain('最近联系人 · 联系人');
    expect(recent?.textContent).toContain('最近群聊 · 群聊');
    expect(document.body.textContent).toContain('研发群');

    await act(async () => { setNativeValue(input!, '张'); });
    await flush(350);

    expect(mocks.fetchFeishuTargets).toHaveBeenCalledWith('user', '张');
...
describe('FeishuForwardModal — 分页失败不吞已加载页（七次复审 P1-2）', () => {
  it('50 people loaded, page2 fails: rows stay visible + tail retry loads the FAILED page', async () => {
    // 弹窗打开时群聊先发一次全量请求 —— 按 type 分流 mock，user
...
describe('FeishuForwardModal — 发送在途期间的选择变更不被快照覆盖（九次复审 P1）', () => {
  ...
  it('a failed target deselected while pending does not get selected again', async () => {
    ...
    expect(row.className).not.toContain('ffm-row--on');
  });
});
...
describe('FeishuForwardModal — 全部成功也保留在途交互（十次复审 P1）', () => {
  ...
  it('searching while pending prevents an old success from closing the picker', async () => {
    mocks.fetchFeishuTargets.mockImplementation(
    ...
    expect(closed).toBe(false);
    expect(document.querySelector<HTMLInputElement>('input[placeholder="搜索用户或群聊"]')?.value)
      .toBe('张三');
    expect(document.querySelector('.ant-tabs')).toBeNull();
  });
...
it('query delivery adapter reuses user/group selection and retries only failed targets', async () => {
  const send = vi.fn().mockResolvedValueOnce({
    success_count: 1, fail_count: 1,
    results: [{ target_id: 'oc-query', ok: true }, { target_id: 'ou-query', ok: false, error: '暂时发送失败' }],
  }).mockResolvedValueOnce({ success_count: 1, fail_count: 0, results: [{ target_id: 'ou-query', ok: true }] });
  mocks.fetchFeishuTargets.mockImplementation(async (type: 'user' | 'chat') => page(type === 'chat'
    ? [target('oc-query', '查询业务群', 'chat')] : [target('ou-query', '查询同事', 'user')]));
  await act(async () => { root.render(<FeishuForwardModal open shareToken="query-snapshot" onClose={() => { closed = true; }} send={send} summary={<p>查询快照 · 完整结果需登录</p>} />); });
  await flush(10); await searchTargets('查询');
  expect(document.body.textContent).toContain('查询快照 · 完整结果需登录');
  const options = Array.from(document.querySelectorAll<HTMLButtonElement>('.ffm-row'));
  expect(options).toHaveLength(2); await click(options[0]); await click(options[1]); await click(findButton('发送（2）')!); await flush(10);
  expect(mocks.forwardShareToFeishu).not.toHaveBeenCalled();
  expect(send.mock.calls[0][0]).toBe('query-snapshot'); expect(send.mock.calls[0][1]).toHaveLength(2);
  expect(findButton('发送（1）')).toBeTruthy(); await click(findButton('发送（1）')!); await flush(10);
  expect(send.mock.calls[1][1]).toEqual([{ target_type: 'user', id: 'ou-query' }]); expect(closed).toBe(true);
});

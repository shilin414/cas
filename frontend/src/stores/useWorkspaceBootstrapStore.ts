...
export interface BootstrapState {
  ...
  /** Patch one task in the shell's recent-task projection. */
  patchTask: (id: string, patch: Partial<TaskSummary>) => void;
  /** Drop one task from the shell's recent-task projection. */
...
let forcedReload: Promise<void> | null = null;
let requestedForceRevision = 0;
let completedForceRevision = 0;
/**
...
/**
 * Separately versions the recent-task projection. A bootstrap response that
 * started before a local rename/delete must not overwrite that successful
 * mutation with its older task snapshot.
 */
let recentTaskRevision = 0;
...
export const useWorkspaceBootstrapStore = create<BootstrapState>()((set, get) => ({
  ...
  load: async (force = false) => {
    if (force) {
      requestedForceRevision += 1;
      invalidationRevision += 1;
      set({ dirty: true });
      if (forcedReload) return forcedReload;
      const forceGeneration = generation;
      const request = (async () => {
        while (completedForceRevision < requestedForceRevision) {
          const targetRevision = requestedForceRevision;
          while (inflight) {
          ...
          if (generation !== forceGeneration) return;
          await get().load(false);
          const refreshError = get().error;
          if (refreshError) throw new Error(refreshError);
          completedForceRevision = targetRevision;
        }
      })().finally(() => {
        if (forcedReload === request) forcedReload = null;
      });
      forcedReload = request;
      return request;
    }
    if (get().loadedAt && !get().dirty) {
    ...
    const myRecentTaskRevision = recentTaskRevision;
    ...
    const request = fetchWorkspaceBootstrap()
      .then((payload) => {
        ...
        set({
          ...
          recentTasks: recentTaskRevision === myRecentTaskRevision
            ? (payload.recent_tasks ?? []).map(adaptTask)
            : get().recentTasks,
  ...
  patchTask: (id, patch) => {
    recentTaskRevision += 1;
    set((state) => ({
      recentTasks: state.recentTasks.map((item) => (
        item.id === id ? { ...item, ...patch } : item)),
    }));
  },
  ...
  removeTask: (id) => {
    recentTaskRevision += 1;
    set((state) => ({
      recentTasks: state.recentTasks.filter((item) => item.id !== id),
    }));
  },

...
let loadingConversationId: number | null = null;
...
export interface RunChatState {
  ...
  /** Drop a deleted conversation from the transcript cache and active pointers. */
...
/** Latest history request, plus runs started while its snapshot was in flight. */
const conversationLoads = new Map<number, { localRunIds: Set<string> }>();
...
export const useRunChatStore = create<RunChatState>()((set, get) => ({
  ...
  loadConversation: async (id: number) => {
    ...
    const request = { localRunIds: new Set<string>() };
    ...
    try {
      ...
      if (!sessionStillCurrent(generation)
        || conversationLoads.get(id) !== request) return;
      const existing = get().conversations[id];
      // The snapshot owns historical messages, but not turns started while it
      // was pending. Preserve their latest local state even if already final.
      ...
      // In particular, the FIRST load must hydrate the prior Q/A even if the
      // user sent before it returned. Match by role + run, since persisted
      // messages have numeric IDs while live bubbles use synthetic IDs.
      const localMessages = (existing?.messages ?? []).filter(
        (message) => message.runId && request.localRunIds.has(message.runId),
      );
      const unmatchedLocal = new Map<string, ChatMessage>(localMessages.map(
        (message) => [`${message.role}:${message.runId}`, message] as const,
      ));
      // Keep server chronology: later turns may already be persisted even
      // though their createRun acknowledgements have not reached us yet.
      const mergedMessages = messages.map((message) => {
        const key = `${message.role}:${message.runId}`;
        const local = unmatchedLocal.get(key);
        unmatchedLocal.delete(key);
        return local ?? message;
      });
      for (const local of unmatchedLocal.values()) {
        // Failed runs can lack a persisted assistant while later users already
        // exist in history. Keep the local outcome beside its own question.
        const userIndex = local.role === 'assistant'
          ? mergedMessages.findIndex((message) => message.role === 'user' && message.runId === local.runId)
          : -1;
        if (userIndex >= 0) mergedMessages.splice(userIndex + 1, 0, local);
        else mergedMessages.push(local);
      }
      conversationLoads.delete(id);
      const ownsLoading = loadingConversationId === id;
      if (ownsLoading) loadingConversationId = null;
      set({
        conversations: {
          ...get().conversations,
          [id]: {
  ...
  removeConversation: (id: number) => {
    ...
    if (cached?.activeRunId) runIds.add(cached.activeRunId);
    cached?.messages.forEach((message) => {
      if (message.runId) runIds.add(message.runId);
    });
    runIds.forEach(closeRunStream);
    ...
    set((state) => {
      ...
      return {
        ...
        lastConversationId: state.lastConversationId === id
          ? null
  ...
  sendMessage: async ({
    ...
    // Keep initial hydration alive: it may be the only copy of prior history.
    // This turn will be merged from local state rather than a stale snapshot.
    conversationLoads.get(cid)?.localRunIds.add(run.id);
    ...
    set((state) => {
      ...
      // History may expose a committed turn before its POST response arrives.
      // Match each role independently: a persisted user must not cause either
      // a duplicate user or the loss of its not-yet-persisted assistant.
      const messages = [...conv.messages];
      for (const incoming of [userMsg, assistantMsg]) {
        const index = messages.findIndex((message) => message.id === incoming.id
          || (message.role === incoming.role && message.runId === run.id));
        if (index === -1) {
          messages.push(incoming);
        } else if (incoming.role === 'assistant') {
          // Streaming and finalization locate this synthetic ID. Keep the
          // already loaded content/status, including a terminal answer.
          messages[index] = { ...messages[index], id: incoming.id };
        } else if (incoming.attachments && !messages[index].attachments) {
          messages[index] = { ...messages[index], attachments: incoming.attachments };
        }
      }
      return {
        conversations: {
...
export function applyEvent(state: RunChatState, event: RunEventRecord): Partial<RunChatState> {
  ...
  switch (event.event_type) {
    ...
    conversations[hostCid] = {
      ...conv,
      messages,
      // A replay for an older terminal run must not clear a newer live run.
      activeRunId: !stillStreaming && conv.activeRunId === runId ? null : conv.activeRunId,
    };
    return { conversations };
  }

function consumeRunEvents(runId: string, generation: number) {
  // Idempotent createRun replay must reuse the existing transport/cursor.
  if (activeStreams.has(runId)) return;
  const stream = openRunStream(runId, {
    onEvent: (event) => {
      // Abort does not retract callbacks already queued by the transport.
      if (activeStreams.get(runId) !== stream) return;
      if (!sessionStillCurrent(generation)) {

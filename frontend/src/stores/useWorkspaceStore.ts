...
interface WorkspaceStore {
  ...
  /** Forget a deleted conversation without disturbing the rest of its workspace. */
...
export const useWorkspaceStore = create<WorkspaceStore>()(
  persist(
    (set, get) => ({
      ...
      forgetConversation: (conversationId) => set((state) => {
        let changed = false;
        const workspaces = Object.fromEntries(
          Object.entries(state.workspaces).map(([applicationId, workspace]) => {
            if (workspace.conversationId !== conversationId) {
              return [applicationId, workspace];
            }
            changed = true;
            return [applicationId, { ...workspace, conversationId: null }];
          }),
        ) as Record<number, ApplicationWorkspaceState>;

...
describe('load', () => {
  ...
  it('force reload drains an in-flight request and returns post-mutation data', async () => {
    ...
    const sharedForced = useWorkspaceBootstrapStore.getState().load(true);
  ...
  it('keeps dirty after a forced reconciliation request fails', async () => {
    mocks.fetchWorkspaceBootstrap.mockRejectedValueOnce({
      response: { data: { detail: 'reconcile failed' } },
    });

    await expect(useWorkspaceBootstrapStore.getState().load(true))
      .rejects.toThrow('reconcile failed');

    expect(useWorkspaceBootstrapStore.getState()).toMatchObject({
      dirty: true,
      isLoading: false,
  ...
  it('runs another fetch when a later mutation arrives during forced reconciliation', async () => {
    let releaseStale!: (value: WorkspaceBootstrap) => void;
    mocks.fetchWorkspaceBootstrap
      .mockReturnValueOnce(new Promise<WorkspaceBootstrap>((resolve) => { releaseStale = resolve; }))
      .mockResolvedValueOnce(payload({ recent_capabilities: [] }));

    const firstMutation = useWorkspaceBootstrapStore.getState().load(true);
    const secondMutation = useWorkspaceBootstrapStore.getState().load(true);
    releaseStale(payload({ recent_capabilities: [summary({ id: 99, name: '已删除' })] }));
...
describe('patch / remove', () => {
  ...
  it('patches and removes task rows from the recent task projection', () => {
    const first = {
      id: '7', title: '旧标题', applicationId: 2,
      createdAt: '2026-09-19T00:00:00Z', updatedAt: '2026-09-19T00:00:00Z',
      executionState: 'idle' as const,
    };
    const second = { ...first, id: '8', title: '保留任务' };
    useWorkspaceBootstrapStore.setState({ recentTasks: [first, second] });

    useWorkspaceBootstrapStore.getState().patchTask('7', {
      title: '新标题', updatedAt: '2026-09-19T01:00:00Z',
    });
    expect(useWorkspaceBootstrapStore.getState().recentTasks[0])
      .toMatchObject({ id: '7', title: '新标题', updatedAt: '2026-09-19T01:00:00Z' });

    useWorkspaceBootstrapStore.getState().removeTask('7');
    expect(useWorkspaceBootstrapStore.getState().recentTasks).toEqual([second]);
  });
});

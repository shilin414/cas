...
describe('useWorkspaceStore (UI state only)', () => {
  ...
  it('forgets a deleted conversation wherever it was remembered', () => {
    const store = useWorkspaceStore.getState();
    store.rememberConversation(1, 101);
    store.setDraft(1, 'keep this draft');
    store.rememberConversation(2, 202);

    useWorkspaceStore.getState().forgetConversation(101);
    ...
    expect(workspaceStateOf(state.workspaces, 2).conversationId).toBe(202);
  });
});

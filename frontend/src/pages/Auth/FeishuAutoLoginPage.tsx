...
import { oauthLoginURL, safeLoginReturnTo } from '@/services/authRedirect';
...
export default function FeishuAutoLoginPage() {
  ...
  useEffect(() => {
    ...
    window.location.href = oauthLoginURL(returnTo);
  }, [returnTo]);
  ...
  return (
    <div style={{ textAlign: 'center', paddingTop: 120 }}>
      ...
      <Button
        type="primary"
        style={{ marginTop: 16 }}
        onClick={() => { window.location.href = oauthLoginURL(returnTo); }}
      >
        使用飞书登录
      </Button>
      <div style={{ marginTop: 32 }}>
        <a href={`${appPath("/login/admin")}?${new URLSearchParams({return_to: returnTo})}`} style={{ color: 'var(--color-text-dim, #aaa)', fontSize: 12 }}>
          管理员登录
        </a>
      </div>
      <Alert

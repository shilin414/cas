...
import { safeLoginReturnTo, sessionLoginRoute } from '@/services/authRedirect';
...
const AdminLoginPage: React.FC = () => {
  ...
  const onFinish = async (values: { username: string; password: string }) => {
    ...
    try {
      ...
      navigate(routerPath(returnTo), { replace: true });
    } catch (error: any) {
  ...
  return (
    <div className="animate-fade-in-scale rounded-xl border border-border bg-card/80 p-8 shadow-lg backdrop-blur-xl">
      ...
      <div className="text-center">
        <a href={appPath(sessionLoginRoute(returnTo))} className="text-text-sec hover:underline" style={{ fontSize: 12 }}>
          返回飞书登录
        </a>
      </div>
    </div>

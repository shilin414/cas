...
export interface UseAccessPolicyEditorResult {
  ...
  setGroupGrants(groupIds: number[], groups: Array<{ id: number; name: string; source_type: string; external_group_type: string; member_count: number; enabled: boolean }>): void;
}
...
export function useAccessPolicyEditor({
  ...
  const save = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const next = await enterpriseApi.updateAccess(saveTargetId, {
        ...
        group_grants: (policy.groups ?? []).map((g) => g.group_id),
      });
  ...
  const setGroupGrants = useCallback((groupIds: number[], groups: Array<{ id: number; name: string; source_type: string; external_group_type: string; member_count: number; enabled: boolean }>) => {
    setPolicy((prev) => prev ? {
      ...prev,
      groups: groupIds.map((id) => {
        const existing = (prev.groups ?? []).find((item) => item.group_id === id);
        if (existing) return existing;
        const group = groups.find((item) => item.id === id);
        return { group_id: id, name: group?.name ?? '', source_type: group?.source_type ?? 'local', external_group_type: group?.external_group_type ?? '', member_count: group?.member_count ?? 0, enabled: group?.enabled ?? true };
      }),
    } : prev);
  }, []);

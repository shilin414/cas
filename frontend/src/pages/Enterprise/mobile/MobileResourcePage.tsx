...
export default function MobileResourcePage({ kind }: { kind: 'chat' | 'fixed' }) {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          await reconcileDeletedApplication(app.id);
          await refresh();
          message.success('已删除');
        } catch {
  ...
  const actions: MobileAction[] = sheetFor ? [
    ...(canManage ? [{ key: 'edit', label: '编辑', icon: <EditOutlined />, onClick: () => openEdit(sheetFor) } as MobileAction] : []),
    ...(canManageAccess ? [{ key: 'access', label: '设置访问权限', icon: <SafetyCertificateOutlined />, onClick: () => navigate(pagePath(`${kind === 'chat' ? 'access/agents' : 'access/apps'}?app=${sheetFor.id}`)) } as MobileAction] : []),
    ...(canManage ? [
      { key: 'toggle', label: sheetFor.enabled !== false ? '停用' : '启用', onClick: () => void toggle(sheetFor, sheetFor.enabled === false) } as MobileAction,
      { key: 'remove', label: '删除', icon: <DeleteOutlined />, danger: true, onClick: () => confirmRemove(sheetFor) } as MobileAction,
    ] : []),
  ] : [];

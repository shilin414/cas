 ...
 *   · department search is server-paged — typing on the departments tab must
...
vi.mock('../../enterpriseApi', () => ({
  enterpriseApi: {
    departmentPage: mocks.departmentPage,
    users: mocks.users,
  },
}));
...
beforeEach(() => {
  mocks.departmentPage.mockReset().mockResolvedValue({ results: [dep(1, '财务部'), dep(2, '技术部')], next_cursor: null });
...
describe('MobileDirectoryPage — tab-split queries (P1-4)', () => {
  it('loads departments by page and searches server-side without requesting users', async () => {
    await mountPage();
    expect(mocks.departmentPage).toHaveBeenCalledTimes(1);
    expect(mocks.users).not.toHaveBeenCalled(); // users tab never entered

    mocks.departmentPage.mockResolvedValueOnce({ results: [dep(1, '财务部')], next_cursor: null });
    ...
    // Server search narrows…
    const names = Array.from(document.querySelectorAll('.mobile-console-row__title'))
      .map((el) => el.textContent);
    expect(names).toEqual(['财务部']);
    expect(mocks.departmentPage).toHaveBeenCalledTimes(2);
    expect(mocks.departmentPage).toHaveBeenLastCalledWith(expect.objectContaining({ q: '财务', limit: 50 }));
...
describe('MobileDirectoryPage — error states (P1-4)', () => {
  it('a departments failure is an error state with retry', async () => {
    mocks.departmentPage.mockRejectedValue(new Error('network down'));
    await mountPage();

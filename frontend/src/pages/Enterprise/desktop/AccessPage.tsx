...
export default function AccessPage({ kind }: { kind: "chat" | "fixed" }) {
  ...
  useEffect(() => { if (typeof enterpriseApi.accessGroups === 'function') void enterpriseApi.accessGroups().then(setAccessGroups); }, []);
  ...
  const {
    ...
    setAccessMode, setDepartmentIds, patchDepartmentGrant, setUserGrants, setGroupGrants,
            ...
            {policy.access_mode === "assigned" && (
              <>
                <div>
                  ...
                  <TreeSelect
                    disabled={!canManage}
                    treeData={buildDepartmentTree(deps)}
                    multiple
                ...
                <div>
                  ...
                  <div style={{ marginTop: 20 }}>
                    <Typography.Title level={5}>授权权限组</Typography.Title>
                    <Select disabled={!canManage} mode="multiple" showSearch optionFilterProp="label"
                      value={(policy.groups ?? []).map((group) => group.group_id)}
                      onChange={(ids) => setGroupGrants(ids, accessGroups)}
                      style={{ width: "100%" }}
                      options={accessGroups.filter((group) => group.enabled).map((group) => ({
                        value: group.id,
                        label: `${group.source_type === 'feishu' ? (group.external_group_type === 'dynamic' ? '飞书·动态' : '飞书') : '本地'} · ${group.name} · ${group.covered_users} 人`,
                      }))}
                      placeholder="选择飞书或本地权限组" />
                  </div>
                  {directoryUsers.error && (
                    <Alert
                      type="error"

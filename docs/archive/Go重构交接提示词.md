# xiaoan-platform — Go 后端重构 · 跨会话交接提示词

你现在负责 **xiaoan-platform 后端 Go 重构的后续开发**。上一会话已完成 G0–G10
...
```text
docs/xiaoan-platform Go 后端目标架构.md        # 目标架构（红线来源）
docs/xiaoan-platform Go 重构执行计划.md        # G0-G15 阶段计划（当前进度 G9 完成）
docs/开发进度清单.md                                 # 全部已验证行为 + 四轮会话记录（必读第六章）
docs/xiaoan-platform 未来演进完整架构文档.md    # 产品与 Domain 终态

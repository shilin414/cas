...
func seedAuthzProvider(t *testing.T, d *sql.DB, key, status string) int64 {
	...
	t.Cleanup(func() {
		_, _ = d.ExecContext(context.Background(), `DELETE FROM providers WHERE id=? AND NOT EXISTS (SELECT 1 FROM runtime_bindings WHERE provider_id=?)`, id, id)
	})
	return id
}
...
func seedAuthzFixture(t *testing.T, d *sql.DB, f authzFixture) int64 {
	...
	t.Cleanup(func() {
		...
		_, _ = d.ExecContext(bg, `DELETE FROM application_department_grants WHERE application_id=?`, appID)
		_, _ = d.ExecContext(bg, `DELETE FROM application_user_grants WHERE application_id=?`, appID)
		_, _ = d.ExecContext(bg, `DELETE FROM application_group_grants WHERE application_id=?`, appID)
		_, _ = d.ExecContext(bg, `DELETE FROM runtime_bindings WHERE application_id=?`, appID)
		_, _ = d.ExecContext(bg, `DELETE FROM applications WHERE id=?`, appID)
	})
	if f.provider == "" {
		return appID
	}

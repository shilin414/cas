...
func TestProviderQueueLossDoesNotIncreaseProviderCapacity(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	provider := slotProvider(t) + "_queue_loss"
	const max = 2
	slots := newTestSlots(t, svc, provider, max, time.Minute)

	handler := &blockingHandler{release: make(chan struct{}), svc: svc, calls: &atomic.Int64{}}
	worker := newSlotWorker(svc, rdb, provider, "queue-loss-worker", handler, slots, 3)
	go worker.Run(ctx)
	...
	// Simulate loss of this fixture provider's Redis wakeups only. Never
	// clear a logical DB: key prefixes do not constrain database-wide commands.
	if !strings.HasPrefix(provider, "itest_slots_") || strings.ContainsAny(provider, ":*?[]") {
		t.Fatal("refusing to delete non-fixture provider queues")
	}
	ownedStreams := []string{execution.QueueStream(rdb, provider)}
	for _, class := range execution.PriorityClasses {
		ownedStreams = append(ownedStreams, execution.PriorityStream(rdb, provider, class))
	}
	if err := rdb.Del(ctx, ownedStreams...).Err(); err != nil {
		t.Fatalf("delete owned fixture streams: %v", err)
	}
	streamKey := execution.PriorityStream(rdb, provider, execution.PriorityClassInteractive)
	if n, err := rdb.XLen(ctx, streamKey).Result(); err != nil || n != 0 {
		t.Fatalf("redis stream messages survived the scoped deletion: len=%d err=%v", n, err)
	}

	// Phase 2: Run C is created after queue loss. Its Redis wakeup is
	// deliberately NOT published. The worker may recreate the empty stream
	// and consumer group after NOGROUP, but that provides no wakeup for C.
	// The MySQL fallback scan must still find C, and durable capacity must
	// still hold it back.
	...
	if got := handler.entered(); got != max {
		t.Fatalf("handler entered %d time(s) after provider queue loss, want %d (A+B only)", got, max)
	}
	if got := handler.maxConcurrent(); got > max {
		t.Fatalf("concurrent handler entries reached %d after Redis loss, want <= %d", got, max)
	}
	if got := activeSlotRows(t, svc, provider); got != max {
		t.Fatalf("MySQL active slots = %d after queue loss, want %d", got, max)
	}

	// Phase 3: releasing the provider frees the capacity; C then runs once.
	// No Redis message is published: every decision below comes from MySQL.
	close(handler.release)
	...
	// Self-healing recreates queue metadata, not run wakeups. The entire
	// post-flush outcome above must still have been driven by MySQL.
	waitUntil(t, "consumer group rebuilt after Redis loss", 5*time.Second, func() bool {
	...
	if n, err := rdb.XLen(ctx, streamKey).Result(); err != nil || n != 0 {
		t.Fatalf("redis run wakeups were repopulated after queue loss: len=%d err=%v", n, err)
	}
}

...
DEPLOYMENT = pathlib.Path(os.environ.get('DEPLOYMENT_CONFIG', ROOT / 'deployment.json'))
BASE_PATH = os.environ.get('APP_BASE_PATH')
if BASE_PATH is None:
    BASE_PATH = json.loads(DEPLOYMENT.read_text(encoding='utf-8'))['basePath']
BASE_PATH = '/' + BASE_PATH.strip('/')
BASE = os.environ.get('E2E_ORIGIN', 'http://localhost:3030').rstrip('/') + BASE_PATH.rstrip('/')
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        context.add_cookies([
            {'name': 'studio_session', 'value': token, 'domain': urlsplit(BASE).hostname, 'path': BASE_PATH.rstrip('/') + '/'},
            {'name': 'studio_csrf', 'value': csrf, 'domain': urlsplit(BASE).hostname, 'path': BASE_PATH.rstrip('/') + '/'},
        ])
        ...
        if not conversation_id:
            ...
            try:
                ...
                diag = page.evaluate(
                    ...
                    f" catalogCount: fetch('{BASE}/api/v2/applications?kind=all&scope=manage')"
                    "   .then(r => r.json()).then(d => d.length) })")
                import json as _json
                catalog_count = page.evaluate(
                    f"fetch('{BASE}/api/v2/applications?kind=all&scope=manage').then(r => r.json()).then(d => d.length)")

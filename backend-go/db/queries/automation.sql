...
INSERT INTO occurrence_delivery_expectations
    (occurrence_id, schedule_delivery_id, channel, sender_identity_mode,
     target_type, target_id, target_name, content_mode, condition_operator, condition_text)
SELECT o.id, d.id, d.channel, d.sender_identity_mode,
       d.target_type, d.target_id, d.target_name, d.content_mode, d.condition_operator, d.condition_text
FROM schedule_occurrences o
...
SELECT occurrence_id, schedule_delivery_id, channel, sender_identity_mode,
       target_type, target_id, target_name, content_mode, created_at, condition_operator, condition_text
...
INSERT INTO schedule_deliveries (schedule_id, channel, sender_identity_mode, target_type,
    target_id, target_name, content_mode, enabled, condition_operator, condition_text)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE
    target_name = VALUES(target_name), content_mode = VALUES(content_mode),
    condition_operator = VALUES(condition_operator), condition_text = VALUES(condition_text),
...
SELECT id, schedule_id, channel, sender_identity_mode, target_type, target_id,
       target_name, content_mode, enabled, created_at, updated_at, condition_operator, condition_text
...
-- name: SkipConditionalDelivery :execresult
-- Retain one terminal execution for EVERY expectation. No outbox on mismatch.
UPDATE delivery_executions
SET status = 'skipped', error_code = 'condition_not_met',
    error_message = 'Final assistant reply did not match delivery condition'
WHERE id = ? AND status = 'pending';

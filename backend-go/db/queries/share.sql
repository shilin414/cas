...
-- name: CreateRunResultShare :exec
INSERT INTO conversation_shares (token, conversation_id, user_id, snapshot, source_run_id)
VALUES (?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE id = id;

-- name: GetRunResultShare :one
SELECT token, snapshot, revoked_at
FROM conversation_shares WHERE source_run_id = ?;

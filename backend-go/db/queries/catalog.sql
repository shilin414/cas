...
WHERE a.id = ? AND a.enabled = 1 AND a.kind = 'chat'
  AND (sqlc.arg('show_all') OR (
    ...
    OR (sqlc.arg('acl_enabled') AND (
      (sqlc.arg('rbac_enabled') AND EXISTS (
        SELECT 1 FROM admin_role_assignments acl_ara
        JOIN admin_roles acl_ar ON acl_ar.id=acl_ara.role_id AND acl_ar.enabled=1
        JOIN admin_role_permissions acl_arp ON acl_arp.role_id=acl_ar.id
        JOIN admin_permissions acl_ap ON acl_ap.id=acl_arp.permission_id AND acl_ap.code='resource.access.bypass'
        JOIN users acl_au ON acl_au.id=acl_ara.user_id AND acl_au.is_active=1
        WHERE acl_ara.user_id=sqlc.arg('acl_bypass_user_id')
          AND acl_ara.enabled=1 AND acl_ara.scope_type='global'
          AND (acl_ara.expires_at IS NULL OR acl_ara.expires_at>CURRENT_TIMESTAMP(3))
      ))
      OR (a.access_mode = 'all' AND NOT EXISTS (
        SELECT 1 FROM directory_users acl_bad
        WHERE acl_bad.local_user_id = sqlc.arg('acl_user_id')
          AND (acl_bad.is_active = 0 OR acl_bad.is_resigned = 1 OR acl_bad.active_status <> 2)
      ))
      OR EXISTS (
      SELECT 1
      ...
      WHERE acl_du.local_user_id = sqlc.arg('acl_user_id')
        ...
        AND (
            ...
            OR EXISTS (
              SELECT 1
              FROM application_group_grants acl_agg
              JOIN access_groups acl_ag ON acl_ag.id = acl_agg.group_id AND acl_ag.enabled = 1
              WHERE acl_agg.application_id = a.id
                AND (
                  EXISTS (SELECT 1 FROM access_group_users acl_agu WHERE acl_agu.group_id = acl_ag.id AND acl_agu.directory_user_id = acl_du.id)
                  OR EXISTS (
                    SELECT 1 FROM access_group_departments acl_agd
                    JOIN directory_department_closure acl_agc ON acl_agc.ancestor_id = acl_agd.department_id AND (acl_agd.include_children = 1 OR acl_agc.depth = 0)
                    JOIN directory_user_departments acl_agud ON acl_agud.department_id = acl_agc.descendant_id
                    WHERE acl_agd.group_id = acl_ag.id AND acl_agud.directory_user_id = acl_du.id
                  )
                  OR EXISTS (SELECT 1 FROM access_group_external_members acl_agem WHERE acl_agem.group_id = acl_ag.id AND acl_agem.directory_user_id = acl_du.id)
                )
            )
          )
      )

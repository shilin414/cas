...
WHERE r.id = ?
  ...
  AND (sqlc.arg('acl_enabled') = 0 OR COALESCE(gate_user.is_staff, 0) = 1 OR (sqlc.arg('rbac_enabled') AND EXISTS (
    ...
    WHERE acl_ara.user_id=r.user_id AND acl_ara.enabled=1 AND acl_ara.scope_type='global'
    ...
    WHERE acl_bad.local_user_id=r.user_id AND (acl_bad.is_active=0 OR acl_bad.is_resigned=1 OR acl_bad.active_status<>2)
  )) OR EXISTS (
    ...
    WHERE acl_du.local_user_id = r.user_id
      ...
      AND a.access_mode='assigned' AND (

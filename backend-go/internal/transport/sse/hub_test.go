...
func TestStaleIdleTimerCannotDeleteTheNewHub(t *testing.T) {
	...
	old.mu.Lock()
	generation := old.idleGeneration
	old.mu.Unlock()
	old.evictIfIdle(generation)
...
// armInitialIdleTimer for AC-4.1.1-5:
//
//	arm timer → GetOrCreate returns → Subscribe → stop the armed timer
//
// The old registry busy-spin followed by waiting for GetOrCreate to return
// established this same order, but a scheduling delay could consume the real
// 20ms IdleTTL before Subscribe. A synctest bubble keeps the clock frozen while
// the test is runnable, so publishing, checking the armed timer, and attaching
// the subscriber cannot accidentally expire the idle window. Sleeps advance
// fake time; Wait settles callbacks before we inspect their effects.
//
// The opposite ordering (Subscribe before armInitialIdleTimer) remains covered
// by TestArmInitialIdleTimerSkipsAlreadySubscribedHub.
func TestSubscriberStopsArmedInitialIdleTimer(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		for round := 0; round < 40; round++ {
			...
			hub := mgr.GetOrCreate(runID)
			if hub == nil || mgr.Lookup(runID) != hub {
				t.Fatalf("round %d: the created hub is not visible in the registry", round)
			}
			hub.mu.Lock()
			armed := hub.idleTimer != nil
			hub.mu.Unlock()
			if !armed {
				t.Fatalf("round %d: GetOrCreate returned without arming the initial idle timer", round)
			}

			sub, ok := hub.Subscribe(StreamProtocolRangeDelta)
			if !ok {
				t.Fatalf("round %d: Subscribe refused a hub before its initial IdleTTL elapsed", round)
			}
			drainAsync(sub)
			hub.mu.Lock()
			stopped := hub.idleTimer == nil
			hub.mu.Unlock()
			if !stopped {
				t.Fatalf("round %d: Subscribe did not stop the armed initial idle timer", round)
			}

			// Keep the subscriber through two complete idle windows, then settle
			// any timer callbacks before asserting that it and its hub survived.
			...
			sub.Close(DropReasonClientGone)
			time.Sleep(opts.IdleTTL)
			synctest.Wait()
			if got := mgr.HubCount(); got != 0 {
				t.Fatalf("round %d: HubCount = %d after the disconnected hub's IdleTTL, want 0", round, got)
			}
			if got := up.ActiveChannels(); got != 0 {
				t.Fatalf("round %d: %d upstream channels remain after idle eviction, want 0", round, got)
			}
		}
	})
}

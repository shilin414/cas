...
func (h *RunHub) runUpstream() {
	...
	h.closeReady()
	defer func() {
		// The forwarder can be blocked sending after the terminal stopped this
		// reader. Closing Redis alone cannot unblock that send.
		h.cancel()
		_ = channel.Close()
		h.mu.Lock()
		h.upstreamUp = false
...
func (h *RunHub) armIdleTimerLocked() {
	...
	h.idleGeneration++
	generation := h.idleGeneration
	h.idleTimer = time.AfterFunc(h.manager.opts.IdleTTL, func() { h.evictIfIdle(generation) })
}
...
func (h *RunHub) evictIfIdle(generation uint64) {
	h.mu.Lock()
	// Stop does not wait for an already-fired callback. A reconnect can
	// start a new idle window while that callback waits for this lock.
	if generation != h.idleGeneration {
		h.mu.Unlock()
		return
	}
	h.idleTimer = nil

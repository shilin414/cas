...
func newShareToken() (string, error) { return sharing.NewToken() }

type shareArtifactRef = sharing.ArtifactRef
type shareMessageEntry = sharing.Entry
...
func (s *Server) GetPublicShare(w http.ResponseWriter, r *http.Request, shareToken string) {
	...
	shared := sharing.Resolve(entries, msgs)
	title := sharing.SnapshotTitle(entries, conv.Title)
	writeJSON(w, http.StatusOK, map[string]any{
		"title":     title,

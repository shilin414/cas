...
import (
	...
	ailyintegration "github.com/shilin414/cas/backend-go/internal/integrations/aily"
)
...
func (s *Server) OauthExchange(w http.ResponseWriter, r *http.Request, params genapi.OauthExchangeParams) {
	...
	if s.Redis != nil {
		_ = s.Redis.Del(r.Context(), ailyintegration.UserAccessTokenCacheKey(s.Redis, result.User.ID)).Err()
	}
...
func (s *Server) clearLegacyRootCookies(w http.ResponseWriter) {
	if s.Config.CookiePath() == "/" {
		return
	}
	for _, name := range []string{s.Store.CookieName(), s.Store.CSRFName()} {
		http.SetCookie(w, &http.Cookie{Name: name, Path: "/", MaxAge: -1,
			HttpOnly: name == s.Store.CookieName(), Secure: s.Config.Session.Secure, SameSite: http.SameSiteLaxMode})
	}
}

func (s *Server) clearSessionCookies(w http.ResponseWriter) {
	s.clearLegacyRootCookies(w)

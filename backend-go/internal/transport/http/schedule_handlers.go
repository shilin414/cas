...
type deliveryRecord struct {
	Condition   schedule.DeliveryCondition `json:"condition"`
	ID          int64                      `json:"id"`
...
type deliveryInput struct {
	Condition   *schedule.DeliveryCondition `json:"condition,omitempty"`
	TargetType  string                      `json:"target_type"`
	TargetID    string                      `json:"target_id"`
	TargetName  string                      `json:"target_name"`
	ContentMode string                      `json:"content_mode"`
...
func (s *Server) GetSchedule(w http.ResponseWriter, r *http.Request, id genapi.ScheduleId) {
	...
	if err != nil {
		...
		writeBare(w, http.StatusInternalServerError, "failed to load automation delivery configuration")
		return
	}
	...
	for _, d := range dels {
		dl = append(dl, deliveryRecord{
			...
			TargetName: d.TargetName, ContentMode: d.ContentMode, Enabled: d.Enabled, Condition: d.Condition,
		})
	}
	// Detail must distinguish an explicitly empty configuration from the list's
	// omitted (unloaded) deliveries. Never let a partial read authorize clearing it.
	writeJSON(w, http.StatusOK, struct {
		scheduleRecord
		Deliveries []deliveryRecord `json:"deliveries"`
	}{scheduleRecord: rec, Deliveries: dl})
}

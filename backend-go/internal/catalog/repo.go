...
func (r *Repo) AccessAllowed(ctx context.Context, appID, userID int64, isStaff bool) (bool, error) {
	...
	resolver := &enterpriseaccess.Service{DB: r.DB, RBACEnabled: r.RBACEnabled}
	allowed, err := resolver.Allowed(ctx, appID, userID, false)
...
func (r *Repo) ListUnboundApplications(ctx context.Context, scope, kind string, callerID int64, isStaff bool) ([]ApplicationWithBinding, error) {
	rows, err := r.q(ctx).ListApplicationsByVisibility(ctx, db.ListApplicationsByVisibilityParams{
		...
		AclUserID: sql.NullInt64{Int64: callerID, Valid: true}, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: uint64(max64(callerID, 0)), Limit: 1000,
	})
	if err != nil {
		return nil, err
	}
...
func (r *Repo) ListApplicationPage(ctx context.Context, q ApplicationPageQuery) ([]ApplicationWithBinding, error) {
	...
	rows, err := r.q(ctx).ListApplicationPage(ctx, db.ListApplicationPageParams{
		...
		RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: uint64(max64(q.CallerID, 0)),
...
func (r *Repo) ResolveMentionCandidates(
	...
	rows, err := r.q(ctx).ResolveMentionCandidates(ctx, db.ResolveMentionCandidatesParams{
		ShowAll: boolArg(isStaff), AclEnabled: boolArg(r.ACLEnabled),
		AclUserID: sql.NullInt64{Int64: callerID, Valid: true}, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: uint64(max64(callerID, 0)),
...
func (r *Repo) BootstrapGroups(ctx context.Context, q BootstrapGroupQuery) (BootstrapGroups, error) {
	...
	aclBypassUser := uint64(max64(q.CallerID, 0))

	def, err := r.q(ctx).BootstrapDefaultApplication(ctx, db.BootstrapDefaultApplicationParams{
		CallerID: caller, ShowAll: showAll, AclEnabled: aclEnabled, AclUserID: caller, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser,
	})
	switch {
	case err == nil:
	...
	if rows, err := r.q(ctx).BootstrapFavoriteApplications(ctx, db.BootstrapFavoriteApplicationsParams{
		CallerID: caller, FavUserID: favUser, ShowAll: showAll, AclEnabled: aclEnabled, AclUserID: caller, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser, Limit: limit,
	}); err != nil {
		return out, err
	} else {
	...
	if rows, err := r.q(ctx).BootstrapFrequentApplications(ctx, db.BootstrapFrequentApplicationsParams{
		CallerID: caller, ShowAll: showAll, AclEnabled: aclEnabled, AclUserID: caller, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser, Limit: limit,
	}); err != nil {
		return out, err
	} else {
	...
	if rows, err := r.q(ctx).BootstrapRecommendedApplications(ctx, db.BootstrapRecommendedApplicationsParams{
		CallerID: caller, ShowAll: showAll, AclEnabled: aclEnabled, AclUserID: caller, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser, FavUserID: favUser, Limit: limit,
	}); err != nil {
		return out, err
	} else {
...
func (r *Repo) BootstrapCategories(ctx context.Context, callerID int64, isStaff bool) (BootstrapCategories, error) {
	...
	aclBypassUser := uint64(max64(callerID, 0))
	agents, err := r.q(ctx).BootstrapAgentCategories(ctx, db.BootstrapAgentCategoriesParams{ShowAll: showAll, AclEnabled: boolArg(r.ACLEnabled), AclUserID: aclUser, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser})
	if err != nil {
		return out, err
	}
	out.Agents = bootstrapAgentCategories(agents)
	apps, err := r.q(ctx).BootstrapAppCategories(ctx, db.BootstrapAppCategoriesParams{ShowAll: showAll, AclEnabled: boolArg(r.ACLEnabled), AclUserID: aclUser, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: aclBypassUser})
	if err != nil {
		return out, err
	}
...
func (r *Repo) ListApplicationRowsByIDs(ctx context.Context, appIDs []int64, callerID int64, isStaff bool) ([]ApplicationWithBinding, error) {
	...
	rows, err := r.q(ctx).ListApplicationRowsByIDs(ctx, db.ListApplicationRowsByIDsParams{
		...
		AclEnabled: boolArg(r.ACLEnabled), AclUserID: sql.NullInt64{Int64: callerID, Valid: true}, RbacEnabled: boolArg(r.RBACEnabled), AclBypassUserID: uint64(max64(callerID, 0)),
	})
	if err != nil {
		return nil, err
	}

...
func (s *Service) AuthorizeExecution(ctx context.Context, applicationID, userID int64, isStaff bool) (*Executable, error) {
	row, err := s.repo().q(ctx).GetExecutionAuthBundle(ctx, db.GetExecutionAuthBundleParams{
		...
		AclEnabled: boolArg(s.ACLEnabled), AclUserID: sql.NullInt64{Int64: userID, Valid: true}, RbacEnabled: boolArg(s.RBACEnabled), AclBypassUserID: uint64(max64(userID, 0)),
	})
...
func (s *Service) RunGateState(ctx context.Context, runID []byte) (*RunGateState, error) {
	row, err := s.repo().q(ctx).GetRunGateState(ctx, db.GetRunGateStateParams{ID: runID, AclEnabled: boolArg(s.ACLEnabled), RbacEnabled: boolArg(s.RBACEnabled)})
	if errors.Is(err, sql.ErrNoRows) {

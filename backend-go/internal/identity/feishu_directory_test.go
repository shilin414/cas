...
func TestListDirectoryUserGroupsAndMembersUseContactV3Contract(t *testing.T) {
	var calls []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls = append(calls, r.URL.Path+"?"+r.URL.RawQuery)
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/open-apis/contact/v3/group/simplelist":
			if r.URL.Query().Get("page_size") != "100" || r.URL.Query().Get("type") != "1" {
				t.Fatalf("group page_size=%s", r.URL.Query().Get("page_size"))
			}
			_ = json.NewEncoder(w).Encode(map[string]any{"code": 0, "msg": "ok", "data": map[string]any{"grouplist": []any{map[string]any{"id": "g-1", "group_id": "custom-1", "name": "AI 高级用户", "description": "动态人群", "member_user_count": 1, "member_department_count": 0, "type": 2}}, "has_more": false}})
		case "/open-apis/contact/v3/group/g-1/member/simplelist":
			if r.URL.Query().Get("member_id_type") != "open_id" || r.URL.Query().Get("member_type") != "user" {
				t.Fatalf("member query=%s", r.URL.RawQuery)
			}
			_ = json.NewEncoder(w).Encode(map[string]any{"code": 0, "msg": "ok", "data": map[string]any{"memberlist": []any{map[string]any{"member_id": "ou-1", "member_type": "user"}}, "has_more": false}})
		default:
	...
	defer srv.Close()
	client := NewFeishuClient(srv.URL, "id", "secret", srv.Client())
	groups, err := client.ListDirectoryUserGroups(context.Background(), "token", 1, "")
	if err != nil {
		t.Fatal(err)
	}
	if len(groups.Groups) != 1 || groups.Groups[0].ID != "g-1" || groups.Groups[0].Type != 2 {
		t.Fatalf("groups=%#v", groups)
	}
	members, err := client.ListDirectoryUserGroupMembers(context.Background(), "token", "g-1", "")
	if err != nil {
		t.Fatal(err)
	}
	if len(members.Members) != 1 || members.Members[0].MemberID != "ou-1" {
...
func TestAuthorizedGETRetriesFeishuRateLimit(t *testing.T) {
	calls := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		w.Header().Set("Content-Type", "application/json")
		if calls == 1 {
			...
			_ = json.NewEncoder(w).Encode(map[string]any{"code": 99991400, "msg": "rate limited", "data": map[string]any{}})
			return
		}
		_ = json.NewEncoder(w).Encode(map[string]any{"code": 0, "msg": "ok", "data": map[string]any{"grouplist": []any{}, "has_more": false}})
	}))
	defer srv.Close()
	client := NewFeishuClient(srv.URL, "id", "secret", srv.Client())
	if _, err := client.ListDirectoryUserGroups(context.Background(), "token", 1, ""); err != nil {
		t.Fatal(err)
	}
	if calls != 2 {

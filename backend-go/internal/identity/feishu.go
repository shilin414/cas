...
func (c *FeishuClient) authorizedGET(ctx context.Context, token, path string) (json.RawMessage, error) {
	for attempt := 0; attempt < 6; attempt++ {
		...
		if err != nil {
			if attempt < 5 {
				if err = waitFeishuRetry(ctx, time.Duration(250*(1<<attempt))*time.Millisecond); err != nil {
					return nil, err
				}
				continue
			}
		...
		transientServer := resp.StatusCode == 500 || resp.StatusCode == 502 || resp.StatusCode == 503 || resp.StatusCode == 504
		if (rateLimited || transientServer) && attempt < 5 {
			...
			if err = waitFeishuRetry(ctx, delay); err != nil {
				return nil, err
			}
			continue
		}
...
func (c *FeishuClient) SendIMMessage(ctx context.Context, token, receiveIDType, receiveID, msgType, content string) error {
	return c.SendIMMessageWithUUID(ctx, token, receiveIDType, receiveID, msgType, content, "")
}

// SendIMMessageWithUUID adds provider-side deduplication for retries of one
// immutable query snapshot/recipient pair; it never switches sender identity.
func (c *FeishuClient) SendIMMessageWithUUID(ctx context.Context, token, receiveIDType, receiveID, msgType, content, uuid string) error {
	body := map[string]any{
	...
	if readErr != nil {
		return fmt.Errorf("feishu: send im: incomplete response")
	}
	var env struct {
		Code *int   `json:"code"`
		Msg  string `json:"msg"`
	...
	if env.Code == nil {
		return fmt.Errorf("feishu: send im: missing success code")
	}
	if *env.Code != 0 {
		return &FeishuAPIError{Path: "/open-apis/im/v1/messages", Code: *env.Code, Msg: env.Msg}
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return fmt.Errorf("feishu: send im: http status %d", resp.StatusCode)
	}
	return nil
}
...
func waitFeishuRetry(ctx context.Context, delay time.Duration) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-time.After(delay):
...
// DirectoryUserGroup is a tenant contact user group mirrored for enterprise ACL.
type DirectoryUserGroup struct {
	...
	Type                  int      `json:"type"`
	DepartmentScopeList   []string `json:"department_scope_list"`
}

type DirectoryUserGroupPage struct {
	Groups    []DirectoryUserGroup
	HasMore   bool
	PageToken string
}

type DirectoryUserGroupMember struct {
	MemberID   string `json:"member_id"`
	MemberType string `json:"member_type"`
}

type DirectoryUserGroupMemberPage struct {
	Members   []DirectoryUserGroupMember
	HasMore   bool
	PageToken string
}

func (c *FeishuClient) ListDirectoryUserGroups(ctx context.Context, token string, groupType int, pageToken string) (*DirectoryUserGroupPage, error) {
	path := "/open-apis/contact/v3/group/simplelist?page_size=100&type=" + strconv.Itoa(groupType)
	if pageToken != "" {
		path += "&page_token=" + url.QueryEscape(pageToken)
	}
	data, err := c.authorizedGET(ctx, token, path)
	...
	var payload struct {
		Groups    []DirectoryUserGroup `json:"grouplist"`
		HasMore   bool                 `json:"has_more"`
		PageToken string               `json:"page_token"`
	}
	if err := json.Unmarshal(data, &payload); err != nil {
		return nil, fmt.Errorf("feishu: directory user groups decode: %w", err)
	}
	return &DirectoryUserGroupPage{Groups: payload.Groups, HasMore: payload.HasMore, PageToken: payload.PageToken}, nil
}

func (c *FeishuClient) ListDirectoryUserGroupMembers(ctx context.Context, token, groupID, pageToken string) (*DirectoryUserGroupMemberPage, error) {
	path := "/open-apis/contact/v3/group/" + url.PathEscape(groupID) + "/member/simplelist?page_size=100&member_type=user&member_id_type=open_id"
	if pageToken != "" {
		path += "&page_token=" + url.QueryEscape(pageToken)
	}
	data, err := c.authorizedGET(ctx, token, path)
	...
	var payload struct {
		Members   []DirectoryUserGroupMember `json:"memberlist"`
		HasMore   bool                       `json:"has_more"`
		PageToken string                     `json:"page_token"`
	}
	if err := json.Unmarshal(data, &payload); err != nil {
		return nil, fmt.Errorf("feishu: directory user group members decode: %w", err)
	}
	return &DirectoryUserGroupMemberPage{Members: payload.Members, HasMore: payload.HasMore, PageToken: payload.PageToken}, nil
}

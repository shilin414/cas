...
func (s *Scheduler) dbNow(ctx context.Context) (time.Time, error) {
	return databaseNow(ctx, s.q(ctx))
}
func databaseNow(ctx context.Context, q db.Querier) (time.Time, error) {
	t, err := q.DBNow(ctx)
	if err != nil {
...
func (s *Scheduler) triggerSchedule(ctx context.Context, schSnapshot *schedule.Schedule, slot, now time.Time) error {
	...
	// A scan can race an edit/another scan. Never admit its stale slot.
	if sch.NextRunAt == nil || !sch.NextRunAt.Equal(slot) {
		return nil
	}
	// Refresh the authoritative instant AFTER waiting for the admission lock.
	now, err = databaseNow(ctx, q)
	if err != nil {
		return err
	}
	if !sch.TriggerConfig.WithinWindow(slot) || !sch.TriggerConfig.WithinWindow(now) {
		if err := s.skipPastInTx(ctx, tx, q, sch, slot, now, "outside effective window"); err != nil {
			return err
		}
		return tx.Commit()
	}
...
func (s *Scheduler) skipPastInTx(ctx context.Context, tx *sql.Tx, q db.Querier, sch *schedule.Schedule, slot, now time.Time, reason string) error {
	if _, err := q.CreateScheduleOccurrence(ctx, db.CreateScheduleOccurrenceParams{
		...
		occ, err := q.GetScheduleOccurrenceBySlot(ctx, db.GetScheduleOccurrenceBySlotParams{ScheduleID: uint64(sch.ID), ScheduledAt: slot})
		if err != nil {
			return err
		}
		if err := captureDeliveryExpectations(ctx, q, occ.ID); err != nil {
...
func (s *Scheduler) TriggerNow(ctx context.Context, scheduleID, userID int64, isStaff bool) (*schedule.Occurrence, error) {
	...
	now = now.Truncate(time.Millisecond)
	if !sch.TriggerConfig.WithinWindow(now) {
		return nil, &schedule.ValidationError{Msg: "run-now is outside trigger starts_at/ends_at"}
	}
...
func (s *Scheduler) admitOne(ctx context.Context, occRow db.ScheduleOccurrence, now time.Time) error {
	...
	currentOcc, err := q.GetScheduleOccurrenceByID(ctx, occRow.ID)
	if err != nil {
		return err
	}
	if currentOcc.Status != schedule.OccPending {
		return nil
	}
	occRow = currentOcc
	now, err = databaseNow(ctx, q)
	...
	s.Log.Info("pending occurrence admitted", "schedule_id", cur.ID, "occurrence_id", occRow.ID)
	return nil
}

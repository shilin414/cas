...
func validateInput(ctx context.Context, in *CreateInput, ownerUserID int64, now func() time.Time, check ApplicationChecker) error {
	...
	if err := in.TriggerConfig.Validate(in.ScheduleType); err != nil {
		return &ValidationError{Msg: err.Error()}
	}
	if in.ScheduleType == TypeOnce {
		if in.RunAt != nil && in.RunAt.Nanosecond()%int(time.Millisecond) != 0 {
			return &ValidationError{Msg: "run_at supports millisecond precision at most"}
		}
		if in.RunAt != nil && !in.TriggerConfig.WithinWindow(*in.RunAt) {
			return &ValidationError{Msg: "run_at must be within trigger starts_at/ends_at"}
		}
...
func (s *Service) Create(ctx context.Context, ownerUserID int64, in *CreateInput) (*Schedule, error) {
	...
	if s.MaxSchedules > 0 {
		...
		if n >= int64(s.MaxSchedules) {
			return nil, &ValidationError{Msg: "自动化数量已达上限"}
		}
	}
...
func (s *Service) replaceDeliveries(ctx context.Context, q db.Querier, scheduleID int64, in []DeliveryInput) error {
	...
	for _, d := range in {
		...
		if _, err := q.UpsertScheduleDelivery(ctx, db.UpsertScheduleDeliveryParams{
			...
			ConditionOperator:  NormalizeCondition(d.Condition).Operator,
			ConditionText:      nullString(NormalizeCondition(d.Condition).Text),
			Enabled:            true,
		}); err != nil {
...
func (s *Service) Update(ctx context.Context, id, userID int64, isStaff bool, in *UpdateInput) (*Schedule, error) {
	...
	if nr.IsZero() && cur.Enabled {
		if _, err := q.SetScheduleEnabled(ctx, db.SetScheduleEnabledParams{Enabled: false, ID: uint64(id)}); err != nil {
			return nil, err
		}
	}
	if in.Deliveries != nil {
...
func (s *Service) SetEnabled(ctx context.Context, id, userID int64, isStaff, enabled bool) (*Schedule, error) {
	...
	if enabled {
		...
		if nr.IsZero() {
			return nil, &ValidationError{Msg: "schedule has no future occurrence within trigger starts_at/ends_at"}
		}
		var nextArg sql.NullTime
		if !nr.IsZero() {
			nextArg = sql.NullTime{Time: nr, Valid: true}
...
func deliveryFromDBRow(r db.ScheduleDelivery) *Delivery {
	return &Delivery{
		...
		Condition:          NormalizeCondition(&DeliveryCondition{Operator: r.ConditionOperator, Text: r.ConditionText.String}),
		Enabled:            r.Enabled,
	}
}
...
func PreviewRuns(in *CreateInput, count int) ([]time.Time, error) {
	...
	if in.ScheduleType == TypeOnce && in.RunAt != nil && !in.TriggerConfig.WithinWindow(*in.RunAt) {
		return nil, &ValidationError{Msg: "run_at must be within trigger starts_at/ends_at"}
	}
	if count < 0 {
		return nil, &ValidationError{Msg: "count must not be negative"}
	}
	in.defaults()
	if !policyValid("scheduleType", in.ScheduleType) {

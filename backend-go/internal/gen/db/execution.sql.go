...
WHERE r.id = ?
  ...
  AND (? = 0 OR COALESCE(gate_user.is_staff, 0) = 1 OR (? AND EXISTS (
...
func (q *Queries) GetRunGateState(ctx context.Context, arg GetRunGateStateParams) (GetRunGateStateRow, error) {
	row := q.db.QueryRowContext(ctx, getRunGateState, arg.ID, arg.AclEnabled, arg.RbacEnabled)

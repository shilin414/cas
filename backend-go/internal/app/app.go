...
import (
	...
	aily "github.com/shilin414/cas/backend-go/internal/integrations/aily"
...
func Build(ctx context.Context, cfg *config.Config) (*App, error) {
	...
	catalogRepo := &catalog.Repo{DB: dbh, ACLEnabled: cfg.Enterprise.ACLEnabled, RBACEnabled: cfg.Enterprise.RBACEnabled}
	catalogSvc := &catalog.Service{
		DB: dbh, Registry: registry, Storage: st,
		ACLEnabled: cfg.Enterprise.ACLEnabled, RBACEnabled: cfg.Enterprise.RBACEnabled,
	}
	...
	rbacSvc := &adminrbac.Service{DB: dbh, Enabled: cfg.Enterprise.RBACEnabled}
	groupSvc := &accessgroup.Service{DB: dbh}
	accessSvc := &enterpriseaccess.Service{DB: dbh, RBACEnabled: cfg.Enterprise.RBACEnabled}
	...
	allowedModelHosts := strings.Split(os.Getenv("AI_MODEL_ALLOWED_HOSTS"), ",")
	aiModels, err := aimodel.NewService(&aimodel.Repo{DB: dbh}, aimodel.Options{EncryptionKey: cfg.Auth.TokenEncryptionKey, AllowedPrivateHosts: allowedModelHosts, DeleteObject: st.Delete})
	if err != nil {
		return nil, fmt.Errorf("initialize AI models: %w", err)
	}
	aiRuntime := modelruntime.NewExecutor(aiModels, st, modelruntime.ExecutorOptions{AllowedPrivateHosts: allowedModelHosts, Log: log})
	return &App{
		AIModels: aiModels, AIModelRuntime: aiRuntime,
		...
		EnterpriseAccess: accessSvc, AdminRBAC: rbacSvc, AccessGroups: groupSvc,
...
// a pool that is gone.
func (a *App) Close() {
	if a.AIModelRuntime != nil {
		a.AIModelRuntime.Close()
	}
	if a.SSEHub != nil {
		a.SSEHub.Close()
	}
